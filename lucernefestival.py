...
class LucernefestivalGrabber:
    def get_event_list(url):
        ...
        with Session() as session:
            ...
            events_list_section = page_data_html_parsed.html.body.find(
                "div", {"class": "list", "id": "event-list"}
            )
            event_list = events_list_section.find_all(
                "div", {"class": "entry", "id": True, "data-date": True}
            )
        return event_list
    ...
    def parse_event_list(event, expanded_details):
        ...
        if not expanded_details:
            ...
            # artists
            artists = (
                event.find("div", {"class": "event-info"})
                .a.get_text()
                .strip()
                .replace(" |", ";")
            )
            ...
            with Session() as inner_session:
                ...
                event_page_data_parsed = BeautifulSoup(
                    event_page_data.content, "html.parser"
                )
                artists_musical_pieces = event_page_data_parsed.find(
                    "div", {"class": "artists-musical-pieces"}
                )
                musical_pieces = artists_musical_pieces.find_all(
                    "div", {"class": "musical-piece"}
                )
                musical_pieces = (
                    artists_musical_pieces.find_all("div", {"class": "with-spaces"})
                    if musical_pieces == []
                    else musical_pieces
                )
                ...
                for musical_piece in musical_pieces:
                    for child in musical_piece.children:
                        work_detail = (
                            child.get_text().strip() if child.get_text() else None
                        )
                        if work_detail:
                            works = (
                                f"{works}{'; ' if child.name == 'strong' else ' '}{work_detail}"
                                if works
                                else work_detail
                            )

                artists_section = artists_musical_pieces.find_all(
                    "div", {"class": "artist"}
                )
                for artist_section in artists_section:
                    for child in artist_section.children:
                        ...
                        if artist_detail:
                            artists = (
                                f"{artists}{'; ' if child.name == 'strong' else ' '}{artist_detail if child.name == 'strong' else f'({artist_detail})'}"
                                if artists
                                else artist_detail
                            )
